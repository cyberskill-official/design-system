...
const banned = JSON.parse(readFileSync(resolve(ROOT, 'scripts/lints/banned-phrases.json'), 'utf8'));
const antiPatterns = JSON.parse(readFileSync(resolve(ROOT, 'scripts/lints/anti-patterns.json'), 'utf8'));
...
function lintA11yNotePresence(file, content) {
  const rel = relative(ROOT, file);
  // Apply to doctrine/part-3*-component spec files (audit-driven assumption: those are the component pages)
  if (!rel.startsWith('doctrine/part-3') && !rel.startsWith('doctrine/part-12')) return;
...
const partFiles = readdirSync(resolve(ROOT, 'doctrine')).filter((f) => f.endsWith('.md'));
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((m = linkRe.exec(lines[i])) !== null) {
      ...
      if (targetBase.endsWith('.md') && !partFiles.includes(targetBase)) {
        // Check if it might be elsewhere (e.g., meta/rfcs/, meta/audits/)
        const candidates = [
          resolve(ROOT, 'doctrine', target),
          resolve(dirname(file), target),
          resolve(ROOT, 'meta', 'rfcs', target),
          resolve(ROOT, 'meta', 'audits', target),
          resolve(ROOT, 'meta', 'runbooks', target),
          resolve(ROOT, 'meta', 'templates', target),
        ];
...
if (opts.json) {
  ...
  out.push('---');
  out.push('*Generated by `scripts/pre-review.mjs` per RFC 2026-005. See `Design System/doctrine/RFCs/2026-005-ai-assisted-contribution-review.md`.*');
