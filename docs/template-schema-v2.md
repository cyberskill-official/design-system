...
Like the visual-diff baselines, this shipped **intentionally incomplete** at first — three exemplars (one per major archetype) proved the pattern end-to-end: `marketing-page` (product), `bod-report` (document), `slide-deck` (deck). It has since been **swept to every hole-driven template** — 26 of 84 templates parametrize their copy via `{{ hole }}` tokens bound in `renderVals()` (the rest hardcode bilingual copy directly per `sc-if` language branch, which this schema format has no slot for yet). All 26 now ship a sidecar; add one to any template that gains hole-driven content as it's touched.
