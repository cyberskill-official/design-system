...
**2. Production React — load the compiled bundle.** Link `styles.css` and `<script src="_ds_bundle.js">`, then read components off the namespace: `const { Button, TextField, … } = window.CyberSkillDesignSystem_847b3c`. This is exactly what `_audit/consumer-smoke-test.html` exercises (and asserts green). Tokens resolve from `styles.css`; components pick up brand tokens automatically.
