...
**Release** — bump `VERSION`, add a line to README's changelog, run the compiler check until clean, then run `_audit/docs-consistency.html` **and** `_audit/namespace-portability.html` (both must be green).
...
- **Responsive** — `_audit/responsive-overflow.html` — 0 document horizontal overflow @390.
...
- **Theme** — `_audit/theme-overflow.html` — forces dark; hard gate = dark-applies + 0 dark overflow @390.

## DOM contrast is unreliable here — theme-contrast is advisory, not a gate (Jul 2026)

An automated *contrast* gate over rendered templates was attempted and **deliberately downgraded to advisory (non-gating)**. A DOM ancestor-walk cannot reliably attribute the actually-painted fg/bg pixel in this system, proven by three false-positive classes: (1) text over a transparent ancestor chain whose real background is a pseudo-element / positioned brand layer (the walk falls back and guesses); (2) email header bands read as a light surface; (3) a deck reporting "black-on-dark-umber" that visually renders as **legible cream** — the walk measured a container's inherited UA `color`, not the styled child that paints the text. Pixel-accurate contrast needs raster, unavailable for nested iframes here. Policy: `theme-overflow.html` gates only on the reliable signals (dark-applies + 0 overflow) and lists solid-bg leaf-text contrast **suspects** for a human to eyeball; the **authoritative** dark-contrast proof stays the manual token audit (`docs/contrast-report.md`) + the `_audit/visual-diff.html` review. Never treat a red DOM-contrast advisory as a defect without checking the actual render.
