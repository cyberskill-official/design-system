...
export function NumberField({ value, onChange, min, max, step = 1, disabled = false, className, children, ...props }) {
  ...
  return (
    <div className={cx("cs-stepper", className)}>
      <button type="button" aria-label="Decrease" disabled={disabled || (min != null && val <= min)} onClick={() => set(val - step)}>−</button>
      <input type="number" value={val} min={min} max={max} step={step} disabled={disabled} onChange={(e) => set(Number(e.target.value))} {...props} />
      <button type="button" aria-label="Increase" disabled={disabled || (max != null && val >= max)} onClick={() => set(val + step)}>+</button>
    </div>
  );
}
