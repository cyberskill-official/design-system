...
- **`docs/`** — `conventions.md` (authoring rules — the three axes: Theme × Element × Language — plus the decision log), `products.md` (element registry), `contrast-report.md` (generated APCA sweep).
...
- Expansion rule: when any set grows (element/variant, icon, component, language), update every deliverable in the same change — tokens, cards + related guidelines, contract trio, all template enums, Identity Lab, docs incl. every related document/README, changelog — and grep the old list to prove it.
