...
- **`tokens/`** — every design decision as a CSS custom property (`--cs-*`): colors, type, spacing, elevation/glass, motion, component tokens, **`elements.css`** — the Ngũ Hành product-identity packs (`data-cs-element="kim|moc|thuy|hoa|tho"` + `data-cs-variant`; role tokens `--cs-accent-*`) — and **`expressions.css`** — the treatment packs (`data-cs-expression="solid|dense|paper|soft|bold|luxe"`; default = liquid-glass). Grammar: **Theme × Element × Expression** are independent; state all three or accept defaults (`light · tho · liquid-glass`). Generated machine-readable exports live at **`tokens/tokens.json`**, **`tokens/tokens.js`** (ESM), and **`tokens/tokens.dtcg.json`** (**W3C DTCG** — `$value`/`$type`, CSS-var keys, scope overrides under `$extensions`; for Tokens Studio for Figma / Style Dictionary) — root tokens grouped by category, plus theme/element/expression override maps — for native/mobile/tooling consumers; regenerate all from `tokens/*.css` after any token change.
- **`docs/`** — `conventions.md` (authoring rules — the four axes: Theme × Element × Expression × Density — plus the decision log), `products.md` (element registry), `contrast-report.md` (generated APCA sweep).
- **`components/`** — 113 exports across 8 groups — Core (Button, Dialog), Forms, Data/Layout, Feedback, Navigation, Overlays, AI-native, and Brand (Logo, Icon, LumiAvatar). Each has a `.d.ts` contract and a `.prompt.md`. In production compose these; in static HTML use the `.cs-*` classes from `styles.css`.
...
- **`guidelines/`** — foundation specimen cards (colors, type, spacing, surfaces, brand, motion, elements, expressions, and Responsive & Bilingual) — handy visual references. Plus **`atomic-view.html`** — a Storybook-style browser of every component grouped by Atomic tier (Atoms/Molecules/Organisms), reskinnable live across Theme × Element × Expression.
